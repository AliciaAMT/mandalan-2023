...
$query=sprintf("select * from inventory where username='%s' and equip>0 order by itemtype desc, weapontype, itemlevel desc, itemrarity desc;",mysql_real_escape_string($username));
...
echo "<table class=\"page\"><tr><td class=\"page\">";
  ...
  echo "<form name=\"keep\" action=\"equipt.php?".time();

  echo "\" method=\"get\"><input class=\"invisible\" type=\"radio\" name=\"keep\" checked=\"checked\" value=\"".$row['itemnumber'];

  echo "\"><br /><input type=\"submit\" value=\"Unequip\" class=\"small\"></form></td></tr><tr><td class=\"page\"><form name=\"trade\" action=\"equipt.php?".time();
...
$query=sprintf("select * from inventory where username='%s' and equip>0 and itemtype='%s' order by itemtype, weapontype, itemlevel desc, itemrarity;",mysql_real_escape_string($username),
