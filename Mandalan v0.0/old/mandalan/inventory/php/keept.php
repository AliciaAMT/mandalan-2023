...
if ($equip!=0)
{
include ('php/equipitem.php');
}
$trade=strip_tags($_GET["trade"]);

include ('php/tradeitem.php');

$equiplh=strip_tags($_GET["equiplh"]);
if ($equiplh!=0)
{
include ('php/equipitemlh.php');
}
$equiprh=strip_tags($_GET["equiprh"]);
if ($equiprh!=0)
{
include ('php/equipitemrh.php');
}
...
$query=sprintf("select * from inventory where username='%s' and keep>0 and itemtype!='Food' order by equipable desc, itemtype desc, weapontype, itemlevel desc, itemrarity desc;",mysql_real_escape_string($username));
...
if ($row['readable']>0)
  ...
  echo "<form name=\"read\" action=\"readt.php?".time();
  ...
  echo "<form name=\"read\" action=\"keylockt.php?".time();
...
if ($row['keylock']==0 and $row['othertype']=="Container" and $row['itemname']!="Tinderbox")
  ...
  echo "<form name=\"read\" action=\"opent.php?".time();
...
echo "<form name=\"equiplh\" action=\"keept.php?".time();

  echo "\" method=\"get\"><input class=\"invisible\" type=\"radio\" name=\"equiplh\" checked=\"checked\" value=\"".$row['itemnumber'];

  echo "\"><br /><input type=\"submit\" value=\"Equip Left\" class=\"small\"></form></td><td class=\"page\"><form name=\"equiprh\" action=\"keept.php?".time();

  echo "\" method=\"get\"><input class=\"invisible\" type=\"radio\" name=\"equiprh\" checked=\"checked\" value=\"".$row['itemnumber'];

  echo "\"><br /><input type=\"submit\" value=\"Equip Right\" class=\"small\"></form></td></tr><tr><td class=\"page\" colspan=\"2\"><form name=\"trade\" action=\"keept.php?".time();
...
if ($row['duality']==0 and $row['equipable']==1)
  ...
  echo "<form name=\"equip\" action=\"keept.php?".time();

  echo "\" method=\"get\"><input class=\"invisible\" type=\"radio\" name=\"equip\" checked=\"checked\" value=\"".$row['itemnumber'];

  echo "\"><br /><input type=\"submit\" value=\"Equip\" class=\"small\"></form></td></tr><tr><td class=\"page\"><form name=\"trade\" action=\"keept.php?".time();
...
$query=sprintf("select * from inventory where username='%s' and keep>0 and itemtype='%s' order by itemtype, weapontype, itemlevel desc, itemrarity;",mysql_real_escape_string($username),
